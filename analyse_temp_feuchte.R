...
# Alle Luecken grau markieren (z.B. 29.05.-30.05. und Ende der Messperiode)
if (nrow(luecken) > 0) {
  luecken_bereiche <- luecken %>%
    mutate(start = zeit - dminutes(diff_min), ende = zeit)
  markiere_luecke <- annotate("rect",
    xmin = luecken_bereiche$start, xmax = luecken_bereiche$ende,
...
plot_ereignis <- function(daten, start, ende, titel, erklaerung, datei) {
  ...
  kombi <- (p1 / p2 / p3 / p4) +
    plot_annotation(
      title = str_wrap(titel, width = 55),
      subtitle = str_wrap(erklaerung, width = 80),
      theme = theme(plot.title = element_text(face = "bold", size = 17),
                    plot.subtitle = element_text(size = 12, colour = "grey30")))
  ggsave(file.path(fig_dir, datei), kombi, width = 12, height = 11,
         dpi = 300, bg = "white")
}
